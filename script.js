...
async function loadModel(path) {
    const model = await tf.loadLayersModel(path);  // Load the model from a given path
    return model;  // Return the loaded model
}

// Function to initialize both models
async function initModels() {
    fossilModel = await loadModel('fossil-classifier-model/model.json');
    checkFossilModel = await loadModel('model-fossil-vs-non/model.json');
