...
function setupApplication(model) {
    ...
    if (dropArea) {
        ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
            dropArea.addEventListener(eventName, (event) => {
                handleDrag(event, model); // Pass the model
            }, false);
        });
    }
    ...
    imageElement.addEventListener('change', () => {
        if (imageElement.files.length > 0) {
            useModel(imageElement.files[0], model);
        }
    });
...
//  Normalization values form Imagenet
const means = tf.tensor1d([0.485, 0.456, 0.406]);
const stds = tf.tensor1d([0.229, 0.224, 0.225]);

// Function to preprocess the image into a tensor
function preprocessImage(image, mean, stds) {
    let tensor = tf.browser.fromPixels(image)  // Convert the image to a tensor
        ...
        //.sub(mean).div(stds)                 // Normalize the image to Normalization values form Imagenet
        /* 
        NOTE: The model was trained on ImageNet normalization values in Pytorch, but it performs better using a [0, 1] range. 
        This could be due to implementation differences introduced during model conversion from PyTorch to TensorFlow and then to TensorFlow.js. 
        */
        .div(tf.scalar(255)) 
...
// Function to initialize (load pre-train models)
async function initModels() {
    /* test the two models */
    const loadModel = await tf.loadLayersModel('public/tfjs_model_pytorch/model.json'); // Pytorch model
    //const loadModel await loadModel('public/tfjs_model/model.json'); // TensorFlow model
    return loadModel
}
...
/* use the model to get the class  */
async function useModel(file, model) {
    ...
    if (file && isImage(file)) { // Ensure isImage is a defined function
        try {
            ...
            const tensor = preprocessImage(image, means, stds); // Preprocess the image (means, stds should be use for the PyTorch model)
